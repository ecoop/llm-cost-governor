...
How to adopt `llm-guardrails` in a Python application. This is the practical companion to the [README](../README.md) — the README explains *what* the library does; this doc explains *how* to wire it into your app.
...
```bash
pip install "llm-guardrails @ git+https://github.com/ecoop/llm-guardrails@v0.1.0"
```

With optional integrations:

```bash
pip install "llm-guardrails[gcs,fastapi,otel] @ git+https://github.com/ecoop/llm-guardrails@v0.1.0"
```
...
from llm_guardrails.state import get_backend, StateBackend
...
| Hook | Module | Purpose |
|---|---|---|
| `ScopeBudgetHook` | `llm_guardrails.budget` | Pre-flight budget enforcement per session/scope |
| `WindowedCapHook` | `llm_guardrails.counters` | Rolling-window cost cap enforcement + alert |
| `EventLogHook` | `llm_guardrails.events` | Structured JSON event per call (stdout) |
| `OTelSpanHook` | `llm_guardrails.otel.hooks` | Per-call OTel span with cost + token attrs |
| `LangSmithMetadataHook` | `llm_guardrails.otel.hooks` | Stamps LangSmith metadata from identity |
...
`MODEL_PRICING` in `llm_guardrails.pricing` covers Anthropic Claude (Fable 5, Opus 5, Opus 4.6/4.7/4.8, Sonnet 5/4.6, Haiku 4.5). Voyage embeddings, OpenAI, Gemini — not included. A call to `usd_for_usage()` with an unrecognized model returns `$0` and fires a one-time "unpriced model" alert through the `AlertSink` protocol.

Two ways to handle in your app:

- **Add pricing rows upstream** — small PR against [`src/llm_guardrails/pricing.py`](../src/llm_guardrails/pricing.py). Preferred; everyone benefits.
...
[`providers/anthropic.py`](../src/llm_guardrails/providers/anthropic.py) is the only shipped adapter. OpenAI, Voyage, Gemini adapters are ~30 lines each following the same shape but aren't in the box until someone needs them.
