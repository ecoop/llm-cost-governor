...
"""Unit tests for llm_cost_governor.otel.hooks + request_span.
...
from llm_cost_governor.otel.hooks import LangSmithMetadataHook, OTelSpanHook
from llm_cost_governor.otel.request_span import request_span
...
def test_otelspanhook_opens_span_in_pre_and_closes_in_post(span_exporter):
    ...
    # Tags passed through as llm_cost_governor.tag.<key>.
    assert span.attributes["llm_cost_governor.tag.stage"] == "drafter"
    assert span.attributes["llm_cost_governor.tag.agent"] == "drafter_cv"
    ...
    assert span.attributes["llm_cost_governor.cost_usd"] == pytest.approx(0.01)
