...
from llm_guardrails.state import StateBackend
    ...
    def record(self, usage: dict, token: str | None = None) -> float:
        ...
        Args:
            usage: Anthropic ``usage``-shaped dict (see
                llm_guardrails.pricing.usd_for_usage).
        ...
        for level, title, body in pending:
            # llm_guardrails.alerts.alert() is best-effort by contract (it
