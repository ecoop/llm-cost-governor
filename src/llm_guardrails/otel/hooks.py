...
class OTelSpanHook:
    ...
    each entry in ``ctx.tags`` as ``llm_guardrails.tag.<key>``.

    `post` sets ``gen_ai.usage.*`` attributes for input/output/cache
    tokens plus ``llm_guardrails.cost_usd`` from the priced UsageRecord,
    ...
    def __init__(self, tracer_name: str = "llm_guardrails") -> None:
    ...
    def pre(self, ctx: CallContext) -> None:
        span = self._tracer.start_span(
            ...
            attributes={
                ...
                **{f"llm_guardrails.tag.{k}": v for k, v in ctx.tags.items()},
            },
        )
    ...
    def post(self, ctx: CallContext, usage: UsageRecord) -> None:
        ...
        span.set_attribute("llm_guardrails.cost_usd", usage.cost_usd)
        span.end()
